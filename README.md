...
每一段关系都可以建立独立工作区。军师分析时，不只看到你刚刚输入的问题，还可以结合工作区里已经沉淀的长期上下文：
...
模拟训练记录会独立保存，训练中出现的虚构内容不会进入真实关系档案或长期记忆。
...
- 只有在需要模型能力时，相关内容才会发送到你自己配置的 OpenAI 兼容模型服务。
- 首次进行 AI 分析前，会展示模型服务地址、模型和发送范围并请求确认。
- 重启服务或更换模型配置后，需要重新确认。

需要注意：**“本地优先”不等于“所有计算都离线”。** 如果你配置的是云端模型服务，自动建档、军师咨询和模拟训练所需的上下文会发送给该模型提供方处理。请根据自己的隐私要求选择模型服务。
